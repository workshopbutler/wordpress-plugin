...
class WSB_Endorsement extends WSB_Page {
    ...
    public function render_content( $attrs = []) {
        $handler = function($endorsement, $template) {
            ...
            return $this->engine->compile_string($html, array('content' => $endorsement->content));
        };
    
        $content = '<p class="wb-endorsement-text">{{ content }}</p>';
    ...
    public function render_author( $attrs = []) {
        $handler = function($endorsement, $template) {
            ...
            $html = do_shortcode($template);
            return $this->engine->compile_string($html, array('endorsement' => $endorsement));
        };
